...
function insert_bitmap() {
    ...
    function _position_image($col_start, $row_start, $x1, $y1,
        ...
        if ($this->size_col($col_start) == 0) { return; }
        if ($this->size_col($col_end)   == 0) { return; }
        if ($this->size_row($row_start) == 0) { return; }
        if ($this->size_row($row_end)   == 0) { return; }

        // Convert the pixel values to the percentage value expected by Excel
        $x1 = $x1     / $this->size_col($col_start) * 1024;
        $y1 = $y1     / $this->size_row($row_start) *  256;
        $x2 = $width  / $this->size_col($col_end)   * 1024;
        $y2 = $height / $this->size_row($row_end)   *  256;
